...
if command -v nvidia-smi >/dev/null 2>&1; then
  ...
  echo "WARNING: nvidia-smi was not found. Training needs a CUDA-enabled JarvisLabs GPU instance."
fi
...
if [ -z "${PYTHON_BIN}" ]; then
  echo "Python 3.10+ not found. Installing Miniconda under ${MINICONDA_DIR}..."
